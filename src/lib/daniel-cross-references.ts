...
import { getCrossReferences, type CrossReferenceRow } from '@/services/crossReferenceService';
...
export async function loadDanielCrossReferences(chapter: number, verse: number) {
  ...
  try {
    const remoteRows = await getCrossReferences(chapter, verse);
    ...
    if (remoteRows.length > 0) {
      ...
      cache.set(cacheKey, remoteCrossReferences);
      return remoteCrossReferences;
    }
  } catch {
    console.warn('Failed to load Daniel cross references from Supabase.');
  }
